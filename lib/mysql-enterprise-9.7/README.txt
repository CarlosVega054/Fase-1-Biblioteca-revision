MySQL Developer Bundle for macOS

INSTALLING

Unpack the downloaded zip bundle by double clicking it in macOS Finder.

This creates a directory containing dmg files for the included MySQL
products, along with zip files for Connector Python (for several
different Python versions) and for Connector J.

Installation is initiated by double clicking the relevant dmg package in
macOS Finder. Products contained in zip files typically require
additional steps.
